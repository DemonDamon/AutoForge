...
class DeepSeekClient(BaseLLMClient):
    ...
    def __init__(self,
        ...
        Args:
            api_key: API密钥，默认从环境变量DEEPSEEK_API_KEY读取
            base_url: API基础URL，默认为DeepSeek的API地址
            model: 使用的模型名称
            organization: 组织ID
        """
        if openai is None:
    ...
    def generate_with_messages(self,
        ...
        try:
            ...
            # DeepSeek Reasoner模型可能会返回reasoning_content
            if hasattr(response.choices[0].message, 'reasoning_content') and response.choices[0].message.reasoning_content:
                logger.info("检测到reasoning_content，包含推理过程")
                # 可以根据需要决定是否返回推理过程
